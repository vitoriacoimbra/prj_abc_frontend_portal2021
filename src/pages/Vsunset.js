...
export default class Vsunset extends Component{
	state={
		processos: [],
    modalIsOpen: false,
    filtro:'',
	...
	loadProcessos = async () =>{
		const {data} = await api.get('/valores/sunset');
  ...
  buscar = async (e) =>{
    ...
    if(!filtro || filtro === '' || filtro === " " || filtro === null){
      ...
      if(!coluna || coluna ==="" || coluna === null){
        ...
        data = await api.get(`/busca/valores?empresa=3&coluna=${coluna}&busca=${filtro}`);
        data = data.data
      }
