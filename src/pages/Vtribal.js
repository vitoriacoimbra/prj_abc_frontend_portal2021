...
export default class Vtribal extends Component{
	...
	loadProcessos = async () =>{
		const {data} = await api.get('/valores/tribal');
  ...
  buscar = async (e) =>{
    ...
    if(!filtro || filtro === '' || filtro === " " || filtro === null){
      ...
      if(!coluna || coluna ==="" || coluna === null){
        ...
        data = await api.get(`/busca/valores?empresa=2&coluna=${coluna}&busca=${filtro}`);
        data = data.data
      }
