...
export default class Modals extends Component{
  ...
  async componentDidUpdate(prevProps, prevState) {
    ...
    if(prevProps.processoId !== this.props.processoId){
      const response = await api.get(`/processos/sunset/${this.props.processoId}`);
    ...
    const id_user = localStorage.getItem('id_user');
      try {
        ...
        if(window.confirm('Tem certeza que deseja gravar esses dados?')){
          await api.put(`/processos/sunset/${this.props.processoId}`, { numAP, numPI, venci, cnpjPrestador, status,numAPAnterior, numPIAnterior, vencimentoAnterior, cnpjPrestadorAnterior, statusAnterior, id_user });
